...
def replace_bn_with_in(module):
    """
    递归地将模型中的 BatchNorm2d 替换为 InstanceNorm2d。
    开启 affine=True 以保留可学习的缩放和平移参数（类似 BN 的 gamma 和 beta）。
    """
...
class ResNetWithDropout(nn.Module):
    def __init__(self, model_name: str, in_channels: int, num_classes: int, dropout_p: float = 0.5):
        ...
        # 🌟 核心修改：在加载完 backbone 后，一键替换所有的 BN 层为 IN 层
        replace_bn_with_in(backbone)
...
class InferenceRunner:
    ...
    def ensure_TKHW(self, x_idx_np: np.ndarray) -> np.ndarray:
        ...
        # (T, 32, 32, K)
        if x_idx_np.shape[1] == 36 and x_idx_np.shape[2] == 32 and x_idx_np.shape[3] == self.topk:
    ...
    def idx_to_onehot_gpu(self, x_idx: torch.Tensor) -> torch.Tensor:
        ...
        # 🌟 找回安全截断：防止因 Padding 数据导致的 index out of bounds 崩溃
        x_idx_safe = torch.clamp(x_idx, min=0, max=self.prompt_token_num - 1)

        x = torch.zeros((t, self.prompt_token_num, H, W), device=x_idx.device, dtype=torch.float16)
        x.scatter_(1, x_idx_safe, 1.0)
        return x
...
def main():
    freq_csv_path = "/home/model_and_data/copy_data_from_3090/wanjie/security_paper/decoding_classification/get_all_attn_decoding/qwen_decoding_50_legal.csv"
    val_csv_path = "/home/model_and_data/copy_data_from_3090/wanjie/security_paper/decoding_classification/get_all_attn_decoding/qwen_decoding_50_legal.csv"

    BIG_NPY_path = "/home/model_and_data/copy_data_from_3090/wanjie/security_paper/decoding_classification/get_all_attn_decoding/legal-qwen-decoding-128_attn_topk128.npy"
    CASEID_PATH_path = "/home/model_and_data/copy_data_from_3090/wanjie/security_paper/decoding_classification/get_all_attn_decoding/legal-qwen-decoding-128_attn_topk128_case_ids.npy"

    model_name = "resnet34"
    topk = 128
    prompt_token_num = 277
    dropout_p = 0.

    ckpt_dir = f"./legal-qwen-decoding_topk64_resnet34"
