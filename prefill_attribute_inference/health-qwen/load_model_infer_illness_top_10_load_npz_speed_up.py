...
class LoadModelInfer(object):
    ...
    def __init__(self):
        self.npz_root = "/home/model_and_data/copy_data_from_3090/wanjie/security_paper_all_experiment_cuda1/prompt_classification/get_all_attn/QWEN3-8B-Instruct/health_short_4_illness_200"
        self.illness_model_path = "/home/model_and_data/copy_data_from_3090/wanjie/security_paper_all_experiment_cuda1/prompt_classification/get_all_attn/QWEN3-8B-Instruct/best_resnet18_not_pca_illness_num_classes4.pt"

        # 输入 val csv
        self.val_csv_path = "/home/model_and_data/copy_data_from_3090/wanjie/security_paper_all_experiment_cuda1/prompt_classification/deal_data/health_short_4_illness_100.csv"

        # 输出：拷贝后的 npz 目录 + 预测结果 csv
        self.out_npz_root = "/home/model_and_data/copy_data_from_3090/wanjie/security_paper_all_experiment_cuda1/prompt_classification/get_all_attn/QWEN3-8B-Instruct/infer_5_val"
        self.out_csv_path = "/home/model_and_data/copy_data_from_3090/wanjie/security_paper_all_experiment_cuda1/prompt_classification/get_all_attn/QWEN3-8B-Instruct/infer_5_val.csv"
    ...
    # ============================================================
    # main: 使用 DataLoader 多进程读取 + 筛选 Top 5 (保证类别多样性)
    # ============================================================
    def load_infer(self, k_top=10, topk_pred=1, num_workers=4):
        ...
        # 实例化我们写的 Dataset
        dataset = NPZDataset(df_val, self.npz_root, k_top=k_top)
        ...
        print("\n-----------------------------------------------------")
        print("开始筛选前 5 条数据（尽量保证每个预测类别都有）...")
        ...
        # 第一轮：遍历按概率排好序的数据，每个预测类别尽量先挑一个最确信的
        for item in all_results:
            if item["pred_label"] not in seen_labels:
                top5_results.append(item)
                seen_labels.add(item["pred_label"])
            # 类别都集齐了就可以提前结束第一轮
            if len(seen_labels) == self.num_classes:
                break

        # 第二轮：如果第一轮挑完还不到 5 个，就从剩下没挑过的数据里补齐概率最高的
        for item in all_results:
            if len(top5_results) >= 5:
                break
            if item not in top5_results:
                top5_results.append(item)

        # 再次按概率降序排序一下，保证最终输出的列表整洁
        top5_results.sort(key=lambda x: x["pred_prob"], reverse=True)

        csv_rows = []

        # 遍历选出的 5 条数据进行拷贝和 CSV 生成
        ...
        print(f"\n✅ Done. 已将选出的 5 个 npz 文件拷贝至: {self.out_npz_root}")
        print(f"✅ 已将选出的 5 条数据写入 csv 文件: {self.out_csv_path}")
